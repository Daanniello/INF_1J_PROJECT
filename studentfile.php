	...
	<?php 
        if (isset($_SESSION['login_docent'])){
            ...
            while ($row1 = mysqli_fetch_assoc($query1)){
                echo "<a href='includes/project/{$row1['Naam']}{$row1['Gebruiker']}.{$row1['Project']}' target='blank'>{$row1['Naam']}</a> &nbsp &nbsp <form action='#' method='post'> Cijfer: <input type='number' name='{$row1['Naam']}'></form> ";
            }
